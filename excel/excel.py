...
class ExcelWork(Excel):
    ...
    def __init__(self, file: str) -> None:
        ...
        while True:
            try:
                ...
                if Confirm.ask("File is in used mode. Is it closed ?", default="(y)", show_default=True):
                    break
                pass

        self._wb: Workbook = load_workbook(filename=self._file, read_only=False, keep_vba=False, data_only=True)
    ...
    def get_row(self, row_num: int) -> Dict[str, str]:
        ...
        for _cell_chr in range(65, 76):
            ...
            if chr(_cell_chr) == 'F':
                _row_data[chr(_cell_chr)] = ",".join(make_data.split_string(self._ws[_cell_idx].value))
                continue
